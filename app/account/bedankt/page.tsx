...
import { CheckCircle, ShoppingBag, Sparkles, Star, ChevronRight, MessageSquare } from "lucide-react";
...
function BedanktContent() {
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      {/* Bedankt-kaart */}
      <div className="bg-white rounded-xl border border-primary-200 p-8 flex flex-col items-center text-center gap-4">
        ...
        <div>
          ...
          {itemName ? (
            <p className="text-sm text-gray-600">
              <span className="font-medium text-gray-800">{itemName}</span> is onderweg naar je toe. Je ontvangt een bevestiging per e-mail.
            </p>
          ) : (
            <p className="text-sm text-gray-600">
      ...
      {/* Upgrade sectie — alleen voor free/trial */}
      {!hasFullAccess && subscription && (
        <div className="bg-white rounded-xl border border-primary-100 p-6 space-y-4">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Met een abonnement krijg je toegang tot de volledige Benji-ervaring — dagelijkse check-ins, reflecties, memories en meer.
            </p>
          </div>
          ...
          <div className="space-y-3">
            {(subType === "free" || subType === "trial") && (
              <Link
                ...
                className="flex items-center justify-between gap-3 p-4 rounded-lg border border-amber-200 bg-amber-50 hover:bg-amber-100 transition-colors group"
              >
            ...
            <Link
              href="/account/abonnement?upgrade=true"
              className="flex items-center justify-between gap-3 p-4 rounded-lg border border-purple-200 bg-purple-50 hover:bg-purple-100 transition-colors group"
            >
              <div className="flex items-center gap-3">
      ...
      {hasFullAccess && (
        <div className="bg-primary-50 rounded-xl border border-primary-100 p-5 text-center">
