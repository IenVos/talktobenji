...
import { Gift, CheckCircle, Clock, Mail, User, Calendar, Package, Copy, Check, FlaskConical, X, Trash2, Key } from "lucide-react";
...
type AccessForm = {
  accessType: "niet_alleen" | "alles_in_1" | "niet_alleen_plus_benji";
  benjiDays: string;
  note: string;
  recipientEmail: string;
};
...
const EMPTY_ACCESS: AccessForm = {
  accessType: "niet_alleen_plus_benji",
  benjiDays: "30",
  note: "",
  recipientEmail: "",
};
...
function AccessCodeModal({ onClose, onCreate }: { onClose: () => void; onCreate: (f: AccessForm) => Promise<string> }) {
  ...
  const inputClass = "w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-400";
  const showDays = form.accessType === "alles_in_1" || form.accessType === "niet_alleen_plus_benji";
      ...
      <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto">
        ...
        {createdCode ? (
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <div>
              <label className="block text-xs text-gray-500 mb-1">Wat geef je toegang tot?</label>
              <select value={form.accessType} onChange={setField("accessType")} required className={inputClass}>
                <option value="niet_alleen">Niet Alleen (e-mailprogramma)</option>
                <option value="alles_in_1">Talk To Benji (chat)</option>
                <option value="niet_alleen_plus_benji">Niet Alleen + Talk To Benji</option>
              </select>
            </div>

            {showDays && (
              <div>
                <label className="block text-xs text-gray-500 mb-1">Aantal dagen Benji-toegang</label>
                <input type="number" value={form.benjiDays} onChange={setField("benjiDays")} required min="1" className={inputClass} />
              </div>
            )}

            <div>
              <label className="block text-xs text-gray-500 mb-1">
                Koppel aan e-mailadres <span className="text-gray-400">(aanbevolen — werkt dan alleen voor dit adres)</span>
              </label>
              <input type="email" value={form.recipientEmail} onChange={setField("recipientEmail")} placeholder="naam@email.nl" className={inputClass} />
            </div>

            <div>
              <label className="block text-xs text-gray-500 mb-1">Notitie voor jezelf <span className="text-gray-400">(optioneel)</span></label>
              <input type="text" value={form.note} onChange={setField("note")} placeholder="Bijv. 'Voor Mirjam uit de workshop'" className={inputClass} />
            </div>
...
export default function CadeaucodesPage() {
  ...
  const adminCreateAccessCode = useAdminMutation(api.giftCodes.adminCreateAccessCode);
  ...
  const handleCreateAccessCode = async (form: AccessForm): Promise<string> => {
    return await adminCreateAccessCode({
      accessType: form.accessType,
      benjiDays: form.accessType !== "niet_alleen" ? parseInt(form.benjiDays, 10) : undefined,
    ...
    <div className="max-w-4xl space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 border border-primary-200 bg-primary-50 rounded-xl text-sm text-primary-700 hover:bg-primary-100 transition-colors font-medium"
          >
            <Key size={15} />
            Toegangscode
          </button>
      ...
      {showAccessModal && (
        <AccessCodeModal
          ...
          onCreate={handleCreateAccessCode}
        />
      )}
    </div>
  );
