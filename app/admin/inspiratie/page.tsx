...
export default function AdminInspiratiePage() {
  ...
  const [form, setForm] = useState({
    title: "",
    content: "",
    type: "gedicht" as (typeof TYPES)[number],
    order: 0,
  ...
  // Afbeelding preview: nieuw bestand, of bestaande URL bij bewerken
  ...
  const handleDelete = async (id: Id<"inspiratieItems">) => {
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-primary-700 mt-1">
          Beheer gedichten, citaten, teksten en PDF&apos;s/ebooks met coverafbeelding.
        </p>
      </div>
          ...
          <div className="mb-6 p-4 rounded-xl bg-primary-50 border border-primary-200 space-y-4">
            ...
            <div className="flex gap-3 flex-wrap">
              <select
                ...
                className="px-3 py-2 border border-primary-200 rounded-lg"
              >
                {TYPES.map((t) => (
            ...
            {isPdf ? (
              <div className="space-y-3">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Afbeelding {isPdf ? "(wordt automatisch uit PDF gehaald)" : "(optioneel)"}</label>
                  <input
                    ref={imageInputRef}
                    type="file"
                    accept="image/*"
                    ...
                    <div className="mt-2 flex items-center gap-3">
                      ...
                      {(imagePreviewUrl || (editingId && editingImageUrl && !form.imageFile)) && (
                        <div className="w-20 h-24 rounded-lg overflow-hidden border border-primary-200 flex-shrink-0 bg-gray-100">
                          ...
                          <img
                            src={imagePreviewUrl ?? editingImageUrl ?? ""}
                            alt="Preview"
              ...
              <div className="space-y-3">
                <textarea
                  placeholder="Inhoud (gedicht, citaat, tekst)"
                  value={form.content}
                  onChange={(e) => setForm((f) => ({ ...f, content: e.target.value }))}
                  rows={4}
                  className="w-full px-3 py-2 border border-primary-200 rounded-lg"
                ...
                <div>
                  ...
                  {editingId && <p className="text-xs text-gray-500 mt-1">Laat leeg om bestaande afbeelding te behouden.</p>}
                </div>
              </div>
            )}
