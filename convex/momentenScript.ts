...
const SCRIPTS: Record<string, string> = {
  scheiding:
    ...
    "- Sluit warm af en toon alleen het BEGIN van de brief als voorproefje (GEEN 'klopt dit?'): leid in met 'Ik ben al met je brief bezig. Zo begint hij:' en zet daarna de openingszin(nen) in de jij-vorm tussen de markeringen [[q]] en [[/q]]. Maak de laatste zin met OPZET niet af en laat hem wegvallen met '...', zodat het duidelijk alleen een begin is en de bezoeker de rest wil ontvangen. Gebruik hun beeld maar herhaal hun woorden niet, voeg iets toe. WEL: [[q]]Elke avond word je stil, en dan begint het zoeken naar wat je anders had kunnen doen. En toch...[[/q]] De tekst zelf moet foutloos en natuurlijk lopen; alleen het slot valt bewust weg in '...' (dus geen kromme of half-afgemaakte woorden, wel een openende zin die op een spannend punt stopt). Zeg daarna kort dat je de hele brief voor ze afmaakt en dat je alleen nog wil weten waar je hem naartoe mag sturen.\n" +
