...
/**
 * Compat-shim: oude checkout-pagina riep deze query aan (LP-slugs per checkout).
 * Blijft bestaan zodat een nog-niet-geüpdatete frontend niet crasht tijdens een
 * deploy. De nieuwe frontend gebruikt checkoutGebruik.perCheckout. Later te verwijderen.
 */
...
export const create = mutation({
  args: {
    ...
    ctaPrijsTekst: v.optional(v.string()),
    ctaMicroCopy: v.optional(v.string()),
    ...
    stickyCtaEnabled: v.optional(v.boolean()),
    stickyCtaText: v.optional(v.string()),
    stickyCtaColor: v.optional(v.string()),
    hideHeader: v.optional(v.boolean()),
    footerText: v.optional(v.string()),
    footerCtaUrl: v.optional(v.string()),
    hideFooterDisclaimer: v.optional(v.boolean()),
    evenHouvastPopupEnabled: v.optional(v.boolean()),
    evenHouvastPopupTekst: v.optional(v.string()),
    evenHouvastPopupKnopTekst: v.optional(v.string()),
    evenHouvastPopupKnopUrl: v.optional(v.string()),
    evenHouvastPopupKnopKleur: v.optional(v.string()),
    ...
    kpHeroKop1: v.optional(v.string()),
    kpHeroKop2: v.optional(v.string()),
    kpHeroTekst: v.optional(v.string()),
    kpHeroSlotzin: v.optional(v.string()),
    kpKeuzeLabel1: v.optional(v.string()),
    kpKeuzeLabel2: v.optional(v.string()),
    kpH2Persoon: v.optional(v.string()),
    kpTekstPersoon: v.optional(v.string()),
    kpCitaatPersoon: v.optional(v.string()),
    kpVoordelenPersoon: v.optional(v.string()),
    kpCtaTekstPersoon: v.optional(v.string()),
  ...
  handler: async (ctx, args) => {
    ...
    if (!existingA) {
      await ctx.db.insert("landingPages", {
        slug: "niet-alleen-a",
        pageTitle: "Niet Alleen — 8 weken begeleiding bij verlies",
        isLive: true,
        heroLabel: "8 weken begeleiding bij verlies en gemis",
        heroTitle: "Je hoeft dit niet alleen te dragen.",
        heroSubtitle: "Om de dag een kleine vraag. Een plek om te schrijven. Iemand die er is.",
        heroBody: "Voor €49 ontvang je 8 weken lang om de dag een persoonlijk bericht in je inbox. Eenmalig, geen abonnement, geen verplichtingen.",
    ...
    if (!existingB) {
      await ctx.db.insert("landingPages", {
        ...
        heroSubtitle: "8 weken. Om de dag een vraag. Een plek die van jou is.",
        heroBody: "Voor €49 ontvang je 8 weken lang om de dag een persoonlijk bericht. Eenmalig, geen abonnement.",
        ctaText: "Start mijn reis",
        ctaUrl: "https://talktobenji.kennis.shop/pay/niet-alleen",
        productImagePath: "/images/niet-alleen-product.png",
...
/**
 * Voeg de vriendelijke "verloop"-tijdlijn toe aan Zij aan Zij (wat je in acht
 * weken kunt verwachten + wat je overhoudt) en hernoem "werkboek" naar "boek".
 */
export const verrijkZijAanZijVerloop = internalMutation({
  args: {},
  handler: async (ctx) => {
    const page = await ctx.db
      .query("landingPages")
    ...
    const hernoem = (s: string | undefined) =>
      (s ?? "").replaceAll("werkboek", "boek").replaceAll("Werkboek", "Boek");

    await ctx.db.patch(page._id, {
      // "werkboek" is geen fijne naam voor iets persoonlijks: overal "boek".
      watJeKrijgtJson: hernoem(page.watJeKrijgtJson),
